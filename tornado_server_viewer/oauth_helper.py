...
def mk_header_with_token(token_key, token_secret):
    app_info = AppInfo('e2hdmubkxyqk5tr', 'jso26ckz6ldg498')
    s = 'OAuth oauth_version="1.0", oauth_signature_method="PLAINTEXT"'
    # s += ', oauth_consumer_key="%s"' % (urllib.quote_plus(app_info.key),)
    s += ', oauth_consumer_key="%s"' % (app_info.key.replace("'",""))

    s += ', oauth_token="%s"' % (token_key.replace("'",""))
    s += ', oauth_signature="%s&%s"' % (app_info.secret.replace("'",""), token_secret.replace("'",""))
    return s
